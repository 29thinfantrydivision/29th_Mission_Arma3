...
if (_selectedAddon isEqualType "") then {
    uiNamespace setVariable ["cba_settings_addon", _selectedAddon];
};

uiNamespace setVariable ["cba_settings_addonIndex", _index];


private _selectedSource = uiNamespace getVariable "cba_settings_source";
...

{
...

    {
        ...
        private _currentValue = (((with uiNamespace do {[cba_settings_clientTemp, cba_settings_missionTemp, cba_settings_serverTemp]}) param [["client", "mission", "server"] find toLower _source, objNull]) getVariable [_setting, [nil, nil]] select 0);
        private _wasEdited = false;

        if (isNil "_currentValue") then {
            _currentValue = [_setting, _source] call cba_settings_fnc_get;
        } else {
            _wasEdited = true;
        };

        private _currentPriority = (((with uiNamespace do {[cba_settings_clientTemp, cba_settings_missionTemp, cba_settings_serverTemp]}) param [["client", "mission", "server"] find toLower _source, objNull]) getVariable [_setting, [nil, nil]] select 1);
        if (isNil "_currentPriority") then {
            _currentPriority = [_setting, _source] call cba_settings_fnc_priority;
        } else {
            _wasEdited = true;
        };


        private _list = ["cba_settings_list", toLower _category, _source] joinString "$";

        private _ctrlOptionsGroup = controlNull;

        if !(_list in _lists) then {
            _ctrlOptionsGroup = _display ctrlCreate ["cba_settings_OptionsGroup", -1, _display displayCtrl 4301];
        ...
        if (_createHeader) then {
            private _ctrlHeaderGroup = _display ctrlCreate ["cba_settings_subCat", -1, _ctrlOptionsGroup];
            private _ctrlHeaderName = _ctrlHeaderGroup controlsGroupCtrl 5010;
            _ctrlHeaderName ctrlSetText format ["%1:", _subCategory];

            private _tablePosY = (_ctrlOptionsGroup getVariable ["cba_settings_tablePosY", ((0.4) * ((((safeZoneW / safeZoneH) min 1.2) / 1.2) / 25))/2]);
            _tablePosY = [_ctrlHeaderGroup, _tablePosY] call _fnc_controlSetTablePosY;
            _ctrlOptionsGroup setVariable ["cba_settings_tablePosY", _tablePosY];
        };


        private _ctrlSettingGroup = switch (toUpper _settingType) do {
            case "CHECKBOX": {
                _display ctrlCreate ["cba_settings_Row_Checkbox", 5000, _ctrlOptionsGroup]
            };
            case "EDITBOX": {
                _display ctrlCreate ["cba_settings_Row_Editbox", 5000, _ctrlOptionsGroup]
            };
            case "LIST": {
                _display ctrlCreate ["cba_settings_Row_List", 5000, _ctrlOptionsGroup]
            };
            case "SLIDER": {
                _display ctrlCreate ["cba_settings_Row_Slider", 5000, _ctrlOptionsGroup]
            };
            case "COLOR": {
                _display ctrlCreate [["cba_settings_Row_Color", "cba_settings_Row_ColorAlpha"] select (count _defaultValue > 3), 5000, _ctrlOptionsGroup]
            };
            case "TIME": {
                _display ctrlCreate ["cba_settings_Row_Time", 5000, _ctrlOptionsGroup]
            };
            default {controlNull};
        };
        ...
        _ctrlDefault ctrlSetTooltip (format ["%1\n%2", localize "STR_cba_settings_default_tooltip", _defaultValueTooltip]);

        _ctrlSettingGroup setVariable ["cba_settings_setting", _setting];
        _ctrlSettingGroup setVariable ["cba_settings_source", _source];
        _ctrlSettingGroup setVariable ["cba_settings_params", _settingData];
        _ctrlSettingGroup setVariable ["cba_settings_groups", _settingControlsGroups];
        ...
        if (_settingType == "LIST") then {
            private _ctrlEmpty = _display ctrlCreate ["cba_settings_Row_Empty", -1, _ctrlOptionsGroup];
            private _height = ((count (_settingData select 0)) * ((((safeZoneW / safeZoneH) min 1.2) / 1.2) / 25)) + ((0.4) * ((((safeZoneW / safeZoneH) min 1.2) / 1.2) / 25));
        ...
        private _script = getText (configFile >> ctrlClassName _ctrlSettingGroup >> "cba_settings_script");
        [_ctrlSettingGroup, _setting, _source, _currentValue, _settingData] call (uiNamespace getVariable _script);


        [_ctrlSettingGroup, _setting, _source, _currentValue, _defaultValue] call cba_settings_fnc_gui_settingDefault;


        [_ctrlSettingGroup, _setting, _source, _currentPriority, _isGlobal] call cba_settings_fnc_gui_settingOverwrite;


        private _enabled = switch (_source) do {
            case "client": {!isServer  && {isNil {cba_settings_userconfig getVariable _setting}}};
            case "mission": {is3DEN  && {isNil {cba_settings_missionConfig getVariable _setting}}};
            case "server": {((isServer || cba_settings_fnc_whitelisted) && {!isNull cba_settings_server})  && {isNil {cba_settings_serverConfig getVariable _setting}}};
        };

        if !(_enabled) then {
            _ctrlSettingName ctrlSetTextColor [1, 1, 1, 0.4];
