...
if (_selectedAddon isEqualType "") then {
    uiNamespace setVariable ["DOTT_settings_addon", _selectedAddon];
};

uiNamespace setVariable ["DOTT_settings_addonIndex", _index];
...
if !(_display getVariable [_selectedAddon, false]) then {
    ...
    private _fnc_controlSetTablePosY = {
        ...
        if (isNull _config) then {_config = missionConfigFile >> ctrlClassName _control};
...

    {
...

        {
            ...
            private _list = ["DOTT_settings_list", toLower _category, _source] joinString "$";
            ...
            private _ctrlSettingGroup = switch (toUpper _settingType) do {
                case "CHECKBOX": {
                    _display ctrlCreate ["DOTT_settings_Row_Checkbox", 5000, _ctrlOptionsGroup]
                };
                case "LIST": {
                    _display ctrlCreate ["DOTT_settings_Row_List", 5000, _ctrlOptionsGroup]
                };
                case "SLIDER": {
            ...
            if (_script == "") then {_script  = getText (missionConfigFile >> ctrlClassName _ctrlSettingGroup >> "cba_settings_script")};
            [_ctrlSettingGroup, _setting, _source, _currentValue, _settingData] call (uiNamespace getVariable _script);


            [_ctrlSettingGroup, _setting, _source, _currentValue, _defaultValue] call DOTT_settings_fnc_gui_settingDefault;
        } forEach ["server"];
