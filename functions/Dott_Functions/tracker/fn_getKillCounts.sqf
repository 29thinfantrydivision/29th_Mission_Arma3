 ...
 *
 * Description:
 * Finds kill count at end of round from events. Units left unconscious at end of round 
 * will have their death credited to whoever knocked them unconscious.
 *
 * Parameter(s): 
 ...
 *
 * Returns:
 * sorted kill Count Array with elements [[unit number, side of unit], numKills]
 *
 * Example:
 * 	  [_events, _sides] call DOTT_tracker_fnc_getKillCounts;
 * 
 */

#include "eventNumbers.hpp"
params["_events", "_sides"];
//[[unit number, side of unit], [instigator number, side of instigator]]
...

{
    ...
    switch (_eventType) do {
        case INFANTRY_KILL_NUM:
        {
            ...
            private _curKills = _killCounts getOrDefaultCall [_key, {0}, true];
            private _addPoints = if (_unitSide == _instigatorSide) then {-1} else {1};
...
_killCounts = [_killCounts, [], {_x select 1}, "DESCEND"] call BIS_fnc_sortBy;
_killCounts