 ...
 * Runs in uiNamespace.
 *
 * NOTE: The global `cba_settings_lock` is used as a mutex
 *       to suppress the LBSelChanged handler while the
 *       updateUI callback programmatically changes the
 *       selection.  It is set to true before lbSetCurSel
 *       and nil'd immediately after.
 *
 * Params:
...
private _ctrlList =
    _controlsGroup controlsGroupCtrl 5110;
...
_ctrlList setVariable [
    "cba_settings_params",
    [_setting, _source, _lbData]
];

_ctrlList ctrlAddEventHandler [
    "LBSelChanged",
    {
        // Bail out while the mutex is held — avoids
        // feedback loops from programmatic selection.
        if (!isNil "cba_settings_lock") exitWith {};

        params ["_ctrlList", "_index"];
        (_ctrlList getVariable
        ...
        private _controlsGroup =
            ctrlParentControlsGroup _ctrlList;
...
// Callback used by the default-reset button.
_controlsGroup setVariable [
    "cba_settings_fnc_updateUI",
    {
        ...
        // Hold the mutex so lbSetCurSel doesn't re-fire
        // the LBSelChanged handler.
        cba_settings_lock = true;
        _ctrlList lbSetCurSel (
            _values find _value
        );
