 ...
 *
 * Purpose: Registers admin chat commands for heal, rearm, reset,
 *          debrief, and goto. Commands dispatch flexibleReset calls
 *          to players via remoteExec, filtered by side.
 *
 * Params:  None (self-registering via DOTT_commands_fnc_addModule)
 * Returns: Nothing
 */

// --- Shared side-resolution infrastructure ---
// Maps lowercase side name -> [engineSide, displayName].
// Available globally so command closures can reference it at call time.
...
// Maps side name -> reset spawn object variable name (as string).
// Used with compile/format to build per-side teleport code blocks.
    ...
    [
            ...
            "reset",
            {
                ...
                if (_stayArg != -1) exitWith
                {
                    ...
                    private _ok = [
                        _sideArg,
                        _code,
                        "Rearming and healing %1 players!"
                    ] call DOTT_loadout_fnc_cmdDispatch;
                ...
                private _side = toLower _argument;
                private _ok = [_side]
                    call DOTT_loadout_fnc_cmdResetTeleport;
        ...
        [
            "debrief",
            {
                ...
                {
                    // Default: teleport everyone to blufor base.
                ...
                {
                    // "here": teleport everyone 15m in front
                    // of the admin.
                ...
                // Timestamp used by baseObjectsInit for
                // Force Parade triggers.
                lastDebriefTime = time;
            }
        ],

        // -------------------------------------------------
        // !goto <side>  (admin-only teleport)
        // -------------------------------------------------
        [
            "goto",
            {
                ...
                private _entry =
                    DOTT_loadout_cmdSideMap get _argument;
                private _baseName =
                    _arsenalBases get _argument;
                ...
                private _baseObj = missionNamespace
                    getVariable [_baseName, objNull];
                [[], false, getPosASL _baseObj]
                    spawn DOTT_loadout_fnc_flexibleReset;
                systemChat format [
                    "Teleporting to %1 spawn!",
                    _displayName
                ];
            }
        ]
