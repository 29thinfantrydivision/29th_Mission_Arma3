 ...
 *
 * Purpose:
 *   Replaces the default OCAP marker event handler with a
 *   customized version. Handles CREATED, UPDATED, and DELETED
 *   marker events, including polyline coordinate conversion and
 *   side-based filtering. Uses CBA event system macros from
 *   the OCAP addon.
 *
 * Parameter(s): None (called via CBA_fnc_waitUntilAndExecute)
 * Returns: Nothing
 *
 * Note:
 *   Heavy macro usage from OCAP's preprocessor defines.
 *   GVAR/EGVAR/EFUNC etc. expand to OCAP_recorder_* names.
 */
...
// Remove the old handler before registering ours.
...
{
    params
        ["_eventType", "_mrk_name", "_mrk_owner",
        "_pos", "_type", "_shape", "_size", "_dir",
        "_brush", "_color", "_alpha", "_text",
        ["_forceGlobal", false],
        ["_creationTime", 0]];
    ...
    switch (_eventType) do
    {
        case "CREATED":
        {
            ...
            {
                OCAPEXTLOG(ARR2(
                    "MARKERALT:CREATE: Processing marker data -- ",
                    _mrk_name));
            };

            // Skip already-tracked markers.
            if (_mrk_name in GVAR(trackedMarkers)) exitWith
            {
                if (GVARMAIN(isDebug)) then
                {
                    OCAPEXTLOG(ARR3(
                        "MARKERALT:CREATE: Marker",
                        _mrk_name,
                        "already tracked, exiting"));
                };
            };
            ...
            {
                format [
                    "CREATE:MARKERALT: Valid CREATED process of %1, sending to extension",
                    _mrk_name] SYSCHAT;
                OCAPEXTLOG(ARR3(
                    "CREATE:MARKERALT: Valid CREATED process of",
                    _mrk_name,
            ...
            // Resolve marker color from config.
            private _mrk_color = "";
            ...
            // Determine marker owner and side.
            private _sideOfMarker = 0;
            ...
            // Force global visibility for civilian-side or
            // system-generated markers.
            if (_sideOfMarker isEqualTo 4
                || (["Projectile#", _mrk_name]
                    call BIS_fnc_inString)
                || (["Detonation#", _mrk_name]
                    call BIS_fnc_inString)
                || (["Mine#", _mrk_name]
                    call BIS_fnc_inString)
                || (["ObjectMarker", _mrk_name]
                    call BIS_fnc_inString)
                || (["moduleCoverMap", _mrk_name]
                    call BIS_fnc_inString)
                || _forceGlobal) then
            {
                _sideOfMarker = -1;
            ...
            // Convert polyline flat coords to paired format.
            ...
            [":MARKER:CREATE:",
                ...
                _captureFrameNo, -1, _mrk_owner,
                _mrk_color, _size, _sideOfMarker,
                _pos, _shape, _alpha, _brush]]
        ...
        case "DELETED":
        {
            if (_mrk_name in GVAR(trackedMarkers)) then
            {
                ...
                {
                    format [
                        "MARKERALT:DELETE: Marker %1 at %2 frame",
                        _mrk_name,
                        GVAR(captureFrameNo)] SYSCHAT;
                    OCAPEXTLOG(ARR3(
                        "MARKERALT:DELETE: Marker",
                        _mrk_name, "deleted"));
                };
