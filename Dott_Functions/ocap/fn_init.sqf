...
// OCAP settings defined in cba_settings.sqf
...
if (isServer) then
{
    ...
    // Initialize handler for all existing sectors.
    {
        _x call DOTT_ocap_fnc_handleSector;
    } forEach (allMissionObjects "ModuleSector_F");

    // Handle sectors that spawn mid-mission.
    addMissionEventHandler ["EntityCreated",
    {
        ...
        if (_entity isKindOf "ModuleSector_F") then
        {
            ...
            {
                params ["_entity"];
                // Delay to let the module fully initialize.
                sleep 10;
    ...
    // When autoStart is off, we manage recording ourselves.
    // Order matters: events won't register if no recording
    // is active yet.
    if !(OCAP_settings_autoStart) then
    {
        ...
        // Trigger a waitAndExecute in OCAP init.sqf so we
        // can dispose of it.
    ...
    // Workaround: saves with zero markers get formatted
    // incorrectly, so create an invisible dummy.
