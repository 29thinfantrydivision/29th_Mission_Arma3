...
//exit script if the number of lives setting should permit unlimited respawns (just in case)
...
//if instead deaths should be stored, record them locally and then exit
...
//if deaths on live is nil, make it zero to prevent errors
if (isNil "DOTT_event_liveDeaths") then
{
	DOTT_event_liveDeaths = 0;
};

//adjust number of deaths by the amount the player had on live
_playerDeaths = (_playerDeaths - DOTT_event_liveDeaths);

//if the number of deaths is equal to or greater then the number of lives
if (_playerDeaths >= DOTT_event_numberOfLives) then
{
	...
	titleText ["<t color='#ffffff' size='4'>Out of Lives!</t>","BLACK OUT",0.5, true, true];
	...
	titleText ["<t color='#ffffff' size='4'>Out of Lives!</t>","BLACK IN",0.5, true, true];
	
	//handle weapon removal for respawning players (if enabled in events)
