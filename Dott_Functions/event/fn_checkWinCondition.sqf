/**
 * DOTT_event_fnc_checkWinCondition
 *
 * Evaluates win conditions each tick during an active round.
 * Supports point-based victory via sector ownership and kill
 * tracking. Conditions can trigger mid-round (loopChecks) or
 * at round end (endChecks).
 *
 * Parameters:
 *     None
 *
 * Returns:
 *     Nothing
 *
 * Requires:
 *     DOTT_event_score (global array)
 *     DOTT_event_opforWinConditions (global)
 *     DOTT_event_bluforWinConditions (global)
 *     DOTT_event_grnforWinConditions (global)
 *     DOTT_event_winCheckInterval (global, seconds)
 *     DOTT_event_fnc_game
 *     DOTT_round_fnc_isRoundActive
 */

private _loopChecks =
    [[{false}], [{false}], [{false}]];
private _endChecks =
    [[{false}], [{false}], [{false}]];

/* --- Register point-scoring event handlers on all objects --- */

{
    private _pointValue =
        _x getVariable ["DOTT_pointValue", 0];

    if (_pointValue == 0) then { continue };

    switch (typeOf _x) do
    {
        case "ModuleSector_F":
        {
            // Sectors award/revoke points on ownership change.
            [_x, "ownerChanged",
                {
                    params [
                        "_sector",
                        "_newOwner",
                        "_oldOwner"
                    ];
                    private _pointValue =
                        _sector getVariable
                            ["DOTT_pointValue", 0];
                    private _newOwnerId =
                        _newOwner call BIS_fnc_sideId;
                    private _oldOwnerId =
                        _oldOwner call BIS_fnc_sideId;

                    if (_newOwnerId <= 2) then
                    {
                        DOTT_event_score set [
                            _newOwnerId,
                            (DOTT_event_score
                                select _newOwnerId)
                                + _pointValue
                        ];
                    };
                    if (_oldOwnerId <= 2) then
                    {
                        DOTT_event_score set [
                            _oldOwnerId,
                            (DOTT_event_score
                                select _oldOwnerId)
                                - _pointValue
                        ];
                    };
                }
            ] call BIS_fnc_addScriptedEventHandler;

            // Credit initial owner at mission start.
            private _owner =
                _x getVariable ["owner", sideUnknown];
            private _idx = _owner call BIS_fnc_sideID;
            if (_idx <= 2) then
            {
                DOTT_event_score set [
                    _idx,
                    (DOTT_event_score select _idx)
                        + _pointValue
                ];
            };
        };
        default
        {
            // Non-sector objects award points on kill.
            _x addEventHandler ["Killed",
            {
                params [
                    "_unit", "_killer", "_instigator"
                ];
                private _pointValue =
                    _unit getVariable
                        ["DOTT_pointValue", 0];
                private _awardTeam =
                    _unit getVariable
                        ["DOTT_awardTeam", sideUnknown];
                private _idx =
                    _awardTeam call BIS_fnc_sideID;
                if (_idx <= 2) then
                {
                    DOTT_event_score set [
                        _idx,
                        (DOTT_event_score select _idx)
                            + _pointValue
                    ];
                };
            }];
        };
    };
} forEach (allMissionObjects "all");

/* --- Win condition check function (points) --- */

fn_numPoints =
{
    params ["_sideId", "_pointsRequired"];

    DOTT_event_score select _sideId >= _pointsRequired
};

/* --- Parse per-side win condition settings --- */

private _sideSettings =
[
    DOTT_event_opforWinConditions,
    DOTT_event_bluforWinConditions,
    DOTT_event_grnforWinConditions
];

{
    // String means no win condition for this side.
    if (_x isEqualType "") then { continue };

    private _winCon = toLower (_x select 0);
    private _winArgs = _x select 1;
    private _atEnd = _x select 2;

    private _checkFn = switch (_winCon) do
    {
        case "points":
        {
            [fn_numPoints, [_forEachIndex, _winArgs]];
        };
        default
        {
            [{false}];
        };
    };

    if (_atEnd) then
    {
        _endChecks set [_forEachIndex, _checkFn];
    }
    else
    {
        _loopChecks set [_forEachIndex, _checkFn];
    };
} forEach _sideSettings;

/* --- End-of-round handler: evaluate endChecks --- */

[
    "DOTT_round_ended",
    {
        private _endChecks = _thisArgs;
        {
            private _fnCheck = _x select 0;
            private _args = [];
            if (count _x >= 2) then
            {
                _args = _x select 1;
            };
            if (_args call _fnCheck) exitWith
            {
                private _winningSide =
                    _forEachIndex call BIS_fnc_sideType;
                [true, _winningSide]
                    call DOTT_event_fnc_game;
            };
        } forEach _endChecks;

        // No side met conditions -- neutral ending.
        [true] call DOTT_event_fnc_game;
    }, _endChecks
] call CBA_fnc_addEventHandlerArgs;

/* --- Mid-round polling loop: evaluate loopChecks --- */

while {call DOTT_round_fnc_isRoundActive} do
{
    // Guard against winCheckInterval being 0 or undefined.
    sleep (DOTT_event_winCheckInterval max 0.5);

    {
        private _fnCheck = _x select 0;
        private _args = [];
        if (count _x >= 2) then
        {
            _args = _x select 1;
        };
        if (_args call _fnCheck) exitWith
        {
            private _winningSide =
                _forEachIndex call BIS_fnc_sideType;
            [true, _winningSide]
                call DOTT_event_fnc_game;
        };
    } forEach _loopChecks;
};
