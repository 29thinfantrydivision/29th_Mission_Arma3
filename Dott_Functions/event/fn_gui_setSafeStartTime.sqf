 ...
 * safe-start countdown timer via a slider or direct text input.
 * The new time is applied via BIS_fnc_countdown and broadcast
 * to all clients as a hint.
 *
 * Parameters:
 *     None
 *
...
// Background declared early so it renders behind everything.
private _bg = _display ctrlCreate ["RscText", 50000];

private _timeCtrl =
    _display ctrlCreate ["DOTT_settings_Row_Time", 5000];
private _sliderCtrl = _display displayCtrl 5140;

/* --- Read layout dimensions from config --- */
...
private _btnCancel =
    _display ctrlCreate ["RscButtonMenuCancel", 5100];
private _wCancel =
    6.2 * (((safezoneW / safezoneH) min 1.2) / 40);
private _hCancel =
    getNumber (configFile >> "RscButtonMenuCancel" >> "h");
...
/* --- OK button: apply the new time --- */

private _btnOK =
    _display ctrlCreate ["RscButtonMenuOK", 5101];
private _sliderPos = ctrlPosition _sliderCtrl;

_btnOK ctrlSetPosition [
    (_sliderPos select 0) + _wSlider - _wCancel,
    0.5,
    _wCancel,
    _hCancel
];
...
_btnOK ctrlAddEventHandler ["ButtonClick",
{
    ...
    {
        systemChat
            "Safe start has already ended! Input ignored.";
    }
    else
    {
...
private _okPos = ctrlPosition _btnOK;
private _bgW =
    (_okPos select 0) + (_okPos select 2)
        - (_cancelPos select 0);
...
/* --- Slider change handler: update h:m:s displays --- */
...
_ctrlSlider ctrlAddEventHandler ["SliderPosChanged",
{
    ...
    (_timeCtrl controlsGroupCtrl 5141) ctrlSetText
        ([floor (_value / 3600), 2]
            call CBA_fnc_formatNumber);
    (_timeCtrl controlsGroupCtrl 5142) ctrlSetText
        ([floor (_value / 60 % 60), 2]
            call CBA_fnc_formatNumber);
    (_timeCtrl controlsGroupCtrl 5143) ctrlSetText
        ([floor (_value % 60), 2]
            call CBA_fnc_formatNumber);
}];

/* --- Time edit fields: sync slider on focus loss --- */

{
    ...
    _ctrlEdit ctrlAddEventHandler ["KillFocus",
    {
        ...
        private _value = round (
            parseNumber ctrlText _ctrlEditHours * 3600
            + parseNumber ctrlText _ctrlEditMinutes * 60
...
/* --- Block Enter key from prematurely closing dialog --- */
