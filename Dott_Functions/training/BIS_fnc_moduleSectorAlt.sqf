/**
 * File: BIS_fnc_moduleSectorAlt.sqf
 * Based On: BIS_fnc_moduleSector (Bohemia Interactive)
 * Modified By: Bae [29th ID]
 *
 * Description:
 *     Modified copy of the vanilla BIS_fnc_moduleSector function.
 *     Custom changes add CBA-configurable capture weights per
 *     vehicle type (TN_costInfantry, TN_costWheeled, TN_costTracked,
 *     TN_costStatic, TN_costWater, TN_costAir), a configurable
 *     capture speed coefficient (TN_captureCoef), optional crew
 *     weight counting (TN_checkCrew), and optional threat-value
 *     modifiers (TN_useThreat). Also adds a client-side warning
 *     when a player is in a vehicle that cannot contribute to
 *     sector capture.
 *
 *     All custom sections are marked with "Custom" comments.
 *     The rest is original BIS code preserved for compatibility.
 *
 * Original Author: Karel Moricky (Bohemia Interactive)
 *
 * Parameters:
 *     See original BIS_fnc_moduleSector documentation.
 *
 * Returns:
 *     Varies by call mode (see original documentation).
 */

private ["_logic","_units","_activated"];
_logic = _this param [0,objnull,[objnull,sideunknown,true]];
_units = _this param [1,[],[[],sideunknown]];
...
switch _mode do {
    ...
    case "server": {
        ...
        // [29th Custom] Fallback defaults when CBA settings
        // haven't loaded (e.g. CBA not present).
        ...
        // [29th Custom] _fnc_threat uses TN_ CBA settings for
        // cost multipliers and optionally factors in config
        // threat values. Also recursively scores crew when
        // TN_checkCrew is enabled for that vehicle class.
        _fnc_threat = {
            ...
            // Non-zero base so even threat [0,0,0] vehicles
            // can still contribute to capture.
            _score = 0.1;
            if (TN_useThreat) then
            {
            ...
            {
                {
                    ...
                    _score = _score + ([_x, TN_costInfantry, false] call _fnc_threat);
        ...
        while {!(isnull _logic) && (simulationenabled _logic) && !(_logic getvariable ["finalized",false])} do {
            ...
            if (str _sides != _sidesOldStr) then {
                ...
                for "_i" from 0 to 5 do {
                    _taskTitle = _taskTitles param [_i,_taskTitle,[""]];
                ...
                _taskDescription = "";
                for "_i" from 0 to 5 do {
                    _taskDescription = _taskDescriptions param [_i,_taskDescription,[""]];
            ...
            //--- Detect leading side
            _owner = sideUnknown;
            // [29th Custom] Use CBA capture speed coefficient
            _timeCoef = TN_captureCoef * (time - _time);
            ...
            if (_totalScore < 0.99999) then {_sideScore set [4,1 - _totalScore];};
            ...
            if (_owner != _ownerOld) then {
                ...
                if (_owner != sideunknown) then {
                    //--- Show it to involved sides and curators
                    _curatorObject = _logic getvariable ["curatorObject",objnull];
                ...
                if !(_firstLoop) then {
                    _ownerID = _owner call bis_fnc_sideID;
                    ...
                    if ({alive _x} count crew _x > 0) then {
                        ...
                        if (_side in _sides) then {
                            ...
                            if !(_x in _units) then {
                                ...
                                // [29th Custom] Vehicle type scoring
                                // uses TN_ CBA settings instead of
                                // hardcoded module attribute values.
        ...
        if (isnull _logic) then {
            ...
            //--- Mark as finalized globally (enableSimulation is local only)
    ...
    case "player": {
        ...
        // [29th Custom] Client-side warning system — alerts
        // players when they are inside a sector in a vehicle
        // type that cannot contribute to capture (e.g. the
        // vehicle's cost weight is 0, or crew weight is
        // disabled for that class). Prevents confusion about
        // why a sector isn't being captured.
        if (isNil "DOTT_sectorlastVic") then
        {
            DOTT_sectorlastVic = "";
        };
        ...
        [_logic] spawn
        {
            ...
            do
            {
                ...
                // Skip if player's side can't capture
                if !(alive player
                    && {(side group player) in _sides}) then
                {
                    continue
                };
                ...
                if (isNull _vehicle
                    || {typeOf _vehicle == DOTT_sectorlastVic}) then
                {
                    continue
                };
                ...
                private _simulation = tolower (gettext
                    (configfile >> "cfgvehicles"
                        >> typeof _vehicle >> "simulation"));

                // Check if player weight inside this vehicle
                // class is actually counted toward capture
                private _checkWarning = switch (_simulation) do
                {
                    case "carx";
                    case "tankx";
                    case "shipx";
                ...
                // If infantry cost is 0, crew weight is
                // irrelevant — the player can't contribute
                // regardless
                _checkWarning = _checkWarning
                    || (TN_costInfantry == 0);
                if !(_checkWarning) then { continue };

                // Check whether the vehicle type itself has
                // a zero cost weight
