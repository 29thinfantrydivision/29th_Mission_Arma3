 ...
 *
 * Description:
 *     Changes or cancels/unforces a currently active safe start.
 *     Will not start a new safe start if one is not already active.
 *     Passing 0 or less unforces the safe start so it ends if teams
 *     are not all ready.
 *
 * Parameters:
 *     _seconds <Number> - New safe start duration in seconds.
