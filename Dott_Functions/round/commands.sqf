/*
 * Function: DOTT_round commands
 *
 * Description:
 *     Registers chat commands for the round module: timer control,
 *     ready/unready, safe start, overtime, and game end.
 *
 * Parameters:
 *     None
 *
 * Returns:
 *     Result of DOTT_commands_fnc_addModule.
 */
    ...
    [
        ...
        [
            "timer",
            {
                ...
                private _minutes =
                    abs (parseNumber _argument);
                [_minutes * 60]
                    call DOTT_round_fnc_setTimer;
                systemChat format [
        ...
        [
            "addtime",
            {
                private _argument = _this select 0;
                private _timeAdd =
                    parseNumber _argument;
                if (
                    [_timeAdd * 60]
                        call DOTT_round_fnc_addTime
                    != -1
                ) then
                {
                    systemChat format [
                        "Adding %1 Minutes to the time limit!",
                        _timeAdd
                    ];
                }
                else
        ...
        [
            "overtime",
            {
                ...
                {
                    [true]
                        call DOTT_round_fnc_setOvertimeEnabled;
                    [_minutes * 60]
                        call DOTT_round_fnc_setOverTimePeriod;
                    systemChat format [
                        "Overtime set for %1 Minutes",
                        _minutes
                    ];
                }
                else
        ...
        [
            "ready",
            {
                private _result = [playerSide, true]
        ...
        [
            "unready",
            {
                private _result = [playerSide, false]
                    call DOTT_round_fnc_manageReady;
                switch (_result) do
                {
                    case 0:
                ...
                if (_minutes > 0) then
                {
                    switch (true) do
                    {
                        ...
                        default
                        {
                            ...
                            [_minutes * 60]
                                call DOTT_round_fnc_changeForcedSafeStart;
                            systemChat
                                "Changing forced safestart!";
                        };
                    };
                }
                else
