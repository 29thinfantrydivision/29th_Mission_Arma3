 ...
 *
 * Description:
 *     Manages timed round events (time warnings). Polls each second
 *     and fires events when the countdown crosses their trigger
 *     thresholds. Handles addTime resets so events re-trigger after
 *     time is added. Client-side only for now (notification display).
 *
 * Parameters:
 *     None
 *
