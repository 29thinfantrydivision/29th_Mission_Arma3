 ...
 *
 * Description:
 *     Changes the ready state of a side and checks if all sides are
 *     ready to start the round. If all sides become ready, initiates
 *     safe start or shortens an existing forced safe start.
 *
 * Parameters:
 ...
 *
 * Returns:
 *     <Number> - 2 if side already in requested state,
 *                1 if round already started,
 *                0 otherwise.
 */
...
private _playerSideReady =
    DOTT_round_sideReady select _sideIdx;
...
["DOTT_round_manageReadyChange", _this]
